...
class LowercasePropertyDefiner extends PropertyDefinerBase {
  ...
  def getValue: String = {
    value.getOrElse(throw new IllegalStateException("key is not initialized"))
  }

  override def getPropertyValue: String =
    value
      .map(_.toLowerCase)
      .getOrElse(throw new IllegalStateException("key is not initialized"))
}

class CredstashPropertyDefiner extends PropertyDefinerBase {

  private val emptyContext = new java.util.HashMap[String, String]()

  private val awsCredentialProvider: AWSCredentialsProvider =
    new DefaultAWSCredentialsProviderChain

  private var awsRegionProvider: AwsRegionProvider =
    new DefaultAwsRegionProviderChain

  private lazy val credstash: JCredStash =
    new JCredStash(
      "credential-store",
  ...
  private var tableName = "credential-store"

  private var credential: Option[String] = None
  ...
  def setAwsRegion(value: String): Unit = {
    awsRegionProvider = new AwsRegionProvider {
      override def getRegion: String = value
    }
  }
  ...
  def setCredential(value: String): Unit = {
    credential = Some(value)
  }

  def getCredential: String = {
    credential.getOrElse(throw new IllegalStateException("key is not initialized"))
  }

  override def getPropertyValue: String = {
    credstash.getSecret(getCredential, emptyContext)
  }
}
