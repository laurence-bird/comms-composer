...
object Rendering {
  ...
  def apply[F[_]](handlebars: HtmlRendering, pdfRendering: PdfRendering[F])(implicit F: Sync[F]) =
    new Rendering[F] {
      override def renderEmail(time: ZonedDateTime,
                               manifest: TemplateManifest,
                               template: Email,
                               emailTemplateData: CommTemplateData): F[Email.Rendered] = {
        val subject = handlebars.render(template.subject, time, emailTemplateData, ???)
        val htmlBody = handlebars.render(template.htmlBody, time, emailTemplateData, ???)
        val textBody =
          template.textBody.map(handlebars.render(_, time, emailTemplateData, ???))
      ...
      override def renderSms(time: ZonedDateTime,
        ...
        val textBody = handlebars.render(template.textBody, time, smsTemplateData, ???)
      ...
      override def renderPrintPdf(html: Print.HtmlBody): F[Print.RenderedPdf] = pdfRendering.apply(html)
    }
}
