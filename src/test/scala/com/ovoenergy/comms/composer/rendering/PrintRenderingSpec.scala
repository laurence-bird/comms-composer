...
class PrintRenderingSpec extends FlatSpec with Matchers with EitherValues {
  ...
  val printTemplate = PrintTemplate[Id](
    header = Some(HandlebarsTemplate(
  ...
  val address = CustomerAddress(line1 = "10 Oxford Street", line2 = "", town = "London", country = "UK", county = "London", postcode = "W1 1AB")

  val renderPrint = PrintRendering.renderPrint(Clock.systemDefaultZone()) _
  ...
  it should "render a template that references fields in the system data" in {
    ...
    val template = PrintTemplate[Id](
      header = Some(HandlebarsTemplate("HEADER {{system.dayOfMonth}}/{{system.month}}/{{system.year}} {{amount}}", requiredFields)),
      body = HandlebarsTemplate("HTML BODY {{system.dayOfMonth}}/{{system.month}}/{{system.year}} {{amount}}", requiredFields),
      footer = Some(HandlebarsTemplate("FOOTER {{system.dayOfMonth}}/{{system.month}}/{{system.year}} {{amount}}", requiredFields))
    )
    val data = Map("amount" -> TemplateData(Coproduct[TemplateData.TD]("1.23")))
    val clock = Clock.fixed(OffsetDateTime.parse("2015-12-31T01:23:00Z").toInstant, ZoneId.of("Europe/London"))

    val result = PrintRendering.renderPrint(clock)(manifest, template, data, address, Some(profile)).right.value
  ...
  it should "render template with each and embedded if using this type reference" in {
    ...
    val result: Either[FailedToRender, RenderedPrint] =
      renderPrint(manifest, template, templateData, address, Some(profile))
    if (result.isLeft) fail(result.left.value.reason)
    else {
      result.right.value.htmlHeader should be(Some("Thanks for your payments of £1.23 (transactionId: 5453ffsdfsdf) £100.23 "))
