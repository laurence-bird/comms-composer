...
class SMSRenderingSpec extends FlatSpec with Matchers with EitherValues {
  ...
  val renderSMS = SMSRenderer.renderSMS(Clock.systemDefaultZone()) _
  ...
  it should "render a template that references fields in the system data" in {
    ...
    val result = SMSRenderer.renderSMS(clock)(manifest, template, data, Some(profile), phoneNumber).right.value
